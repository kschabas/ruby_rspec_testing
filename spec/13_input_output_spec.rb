...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    subject(:game_not_over) { described_class.new(4, '5') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        bad_value2 = '@'
        good_value = '6'
        allow(game_loop).to receive(:player_input).and_return(bad_value1, bad_value2, good_value)
      end
  ...
  describe '#final_message' do
    ...
    # @guess, and @count
    subject(:game) { described_class.new(2, '2', 3) }
    ...
    # Write a test for the following context.
    subject(:long_game) { described_class.new(1, '1', 4)}
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        correct_phrase = "That was hard. It took you 4 guesses!\n"
        expect{ long_game.final_message}.to output(correct_phrase).to_stdout
      end
    end
  end
end
