...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('11', '3')
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('11', 'cs', '3')
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        input = 3
        min = verify_input_game.instance_variable_get(:@mininum).to_i
        max = verify_input_game.instance_variable_get(:@maximum).to_i
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_hundred2) { described_class.new(100, 600) }
      it 'returns 9' do
        max = game_hundred2.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:display_turn_order_game) { described_class.new(1, 100) }
    let(:bstree) { instance_double(BinarySearch, min: 1, max: 100, guess: 50) }

    before do
      # You'll need to create a few method stubs.
      allow(bstree).to receive(:make_guess)
      allow(bstree).to receive(:update_range)
      allow(display_turn_order_game).to receive(:display_guess).with(bstree)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { display_turn_order_game.display_turn_order(bstree) }.to change { display_turn_order_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(bstree).to receive(:make_guess).once
      display_turn_order_game.display_turn_order(bstree)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(bstree).to receive(:update_range).once
      display_turn_order_game.display_turn_order(bstree)
    end
