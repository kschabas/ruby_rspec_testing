...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:fn2) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        result = fn2.make_guess
        expect(result).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:fn3) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        result = fn3.make_guess
        expect(result).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:fn4) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        result = fn4.make_guess
        expect(result).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:fn5) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        result = fn5.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      guess = 2
      let(:ng) { double('random_number')}
      subject(:game_over_test) { described_class.new(0, 9, ng, guess) }
      ...
      it 'is game over' do
        allow(ng).to receive(:value).and_return(guess)
        expect(game_over_test).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        allow(ng).to receive(:value).and_return(guess + 1)
        expect(game_over_test).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:game_corner_case) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        game_corner_case.update_range
        expect(game_corner_case.min).to eq(game_corner_case.max)
      end

      it 'does not update max' do
        game_corner_case.update_range
        expect(game_corner_case.max).to eq(8)
      end
    end
  end
end
