...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:test_beverage) { described_class.new('sprite') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(test_beverage.type).to eq('sprite')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(test_beverage.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:beer) { described_class.new('beer') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(beer).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:wine) { described_class.new('wine', 8) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(wine).not_to be_full
      end
    end
  end
end
